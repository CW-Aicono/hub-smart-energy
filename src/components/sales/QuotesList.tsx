...
import { FileText, Download, Loader2, Plus, Share2, CheckCircle2, XCircle, Eye } from "lucide-react";
...
export function QuotesList({ projectId, onCreate, reloadKey = 0 }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    const { data, error } = await supabase
      .from("sales_quotes")
      .select("id, version, total_einmalig, modul_summe_monatlich, pdf_storage_path, signed_at, rejected_at, viewed_at, public_token, signer_name, created_at")
