...
export interface SalesRulesManagerProps {
  scope: "global" | "partner";
  partnerId?: string | null;
  canManage?: boolean;
}

export function SalesRulesManager({ scope, partnerId, canManage = true }: SalesRulesManagerProps) {
  ...
  const selectableDevices = scope === "partner"
    ? devices.filter((d) =>
        d.owner_scope === "global" || (d.owner_scope === "partner" && d.partner_id === partnerId),
      )
    : devices;
  ...
  const headerTitle = scope === "partner" ? "Sales – Eigene Auswahl-Regeln" : "Sales – Auswahl-Regeln";
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        {canManage && (
          <Button onClick={openNew} disabled={selectableDevices.length === 0}>
