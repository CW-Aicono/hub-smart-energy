...
export function FloorPlan3DViewer({ floor, locationId, sensors = [], isAdmin = false, compact = false, readOnly = false }: FloorPlan3DViewerProps) {
  ...
  return (
    <div className={compact ? "h-full w-full" : "flex flex-col h-full w-full overflow-hidden"}>
      ...
      {/* 3D Canvas */}
      <div className={`${compact ? "h-full w-full" : "flex-1 relative min-h-0"} bg-gradient-to-b from-muted/30 to-muted/60`}>
        {loading ? (
