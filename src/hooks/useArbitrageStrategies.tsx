...
export function useArbitrageStrategies() {
  ...
  const createStrategy = useMutation({
    mutationFn: async (values: {
      ...
      const { error } = await supabase.from("arbitrage_strategies").insert({ ...values, tenant_id: tenantId! });
      if (error) throw error;
    },
    onSuccess: () => {
  ...
  return { strategies, isLoading, createStrategy, updateStrategy, deleteStrategy };
}
